...
TRACCC_HOST_DEVICE inline void gbts_bin_spacepoints(
  ...
  for (unsigned int globalIndex = globalIdx; globalIndex < payload.nSp;
    ...
    // Here we concatenate the eta bin, order-preserving phi bits, and the spacepoint index
    // into a single 64-bit integer. Which is then used to sort the nodes in the next stage.
