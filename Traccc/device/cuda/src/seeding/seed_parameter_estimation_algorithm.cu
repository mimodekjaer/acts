...
void seed_parameter_estimation_algorithm::estimate_seed_params_kernel(
  ...
  // One warp per block: the kernel is latency bound (one seed per thread,
  // with a long chain of dependent loads and transcendental functions), so
  // spreading the few thousand seeds over as many SMs as possible helps.
  const unsigned int n_threads = warp_size();
